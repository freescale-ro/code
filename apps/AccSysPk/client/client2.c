	...
	int t=0, res, sock, bytes_recieved;	
	char b[10]="#";
      	char s1[1024]="";
      	char s2[1024]="";
	...
	struct hostent *host;
		...
		//cheia publica din fisier a serverului
		...
		//cheia privata din fisier a clientului 2
        	FILE *f1=fopen("newkey2.pem","rb");
        	fseek(f1,0,SEEK_END);
		...
		while(1)
		{
			...
			//separare user si parola prin ' ' si stocare in s1 respectiv s2
			...
			printf("\nUserID+Password : %s",decryptionhex);
